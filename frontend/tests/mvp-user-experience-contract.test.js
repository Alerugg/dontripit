...
test('price UI refuses to invent missing values', () => {
  ...
  assert.match(printPage, /if \(!primary\) return null/)
  assert.match(printPage, /positiveNumber/)
  assert.match(printPage, /No reutilizamos el precio de otra edición/)
  assert.doesNotMatch(printPage, /value \|\| 0/)
  assert.doesNotMatch(printPage, /Sin enlace Cardmarket exacto disponible/)
