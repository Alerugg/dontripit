...
test('collector-first Editorial Home stays truthful and does not import mock prices', () => {
  ...
  assert.match(search, /game: selectedGame/)
  assert.doesNotMatch(home, /312,40|68,95|3\.240|12480|Pikachu · Reverse/)
})

test('launch front has explicit responsive layouts for Editorial Home, game hubs and dashboard', () => {
  const home = source('app/canva-workspace.css')
  const editorial = source('app/lovable-v4-home-editorial.css')
  ...
  assert.match(editorial, /@media\(max-width:980px\)/)
  assert.match(editorial, /@media\(max-width:680px\)/)
  assert.match(editorial, /@media\(max-width:430px\)/)
...
test('home game selector remains keyboard and screen-reader explicit', () => {
  ...
  assert.match(search, /ACTIVE_GAME_CATALOG\.map/)
})
