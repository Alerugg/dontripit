...
TARGETS={'BE2':'4870','BE02':'4756','309':'4885','SK2':'4889'}
...
def main():
 ...
 for code,exp in TARGETS.items():
  ...
  reports.append({'set_code':code,'candidate_expansion_code':f'{code}-JP','candidate_id':exp,'certified_idExpansion':exp if r['status']=='certified' else None,'status':r['status'],'candidate':r})
  time.sleep(1.25)
 payload={'source':'cardmarket','mode':'read_only','method':'global_surface_candidate_plus_first_party_cardmarket_image_s3_binary_signature','production_writes':0,'certified':sum(r['status']=='certified' for r in reports),'inconclusive':sum(r['status']=='inconclusive' for r in reports),'not_certified':sum(r['status']=='not_certified' for r in reports),'results':reports}
 Path('/tmp/yugioh-ocg-classic-expansions-v10.json').write_text(json.dumps(payload,indent=2,sort_keys=True)+'\n',encoding='utf-8'); print(json.dumps(payload,indent=2,sort_keys=True)); return 0
