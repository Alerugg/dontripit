...
GAME='yugioh'; ACCEPTED=('accepted','mapped','exact'); METHOD='cardmarket_ocg_certified_public_code_singleton_v1'; EXPECTED=219
SURFACES={'4569':('DP23',56),'4580':('DP22',56),'4604':('DP21',56),'4633':('DP19',51)}
...
def main()->int:
 ...
 conn=psycopg2.connect(url,connect_timeout=30,application_name='dontripit_ygo_ocg_duelist_packs_proof_v1'); conn.set_session(readonly=True,autocommit=False)
 try:
 ...
 if len(links)!=EXPECTED or len({int(r['external_product_id']) for r in links})!=EXPECTED or len({int(r['print_id']) for r in links})!=EXPECTED: failures.append('global_identity_not_219_one_to_one')
 ...
 out=Path(os.getenv('YGO_OCG_DUELIST_PACKS_PROOF_OUTPUT','/tmp/yugioh-ocg-duelist-packs-production-v1.json')); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)+'\n',encoding='utf-8'); print(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)); return 0 if not failures else 2
