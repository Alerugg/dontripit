...
EXPECTED_METHODS = {
    ...
    "cardmarket_ocg_certified_typo_image_bijection_v1": 3,
}
...
def main() -> int:
    ...
    report["proof_version"] = 4
    report["expected_mapping_methods"] = EXPECTED_METHODS
    report["failures"] = failures
    report["status"] = "pass" if not failures else "fail"
    report["production_writes"] = 0

    output = os.getenv("YGO_AGOV_JP_PRODUCTION_PROOF_V4_OUTPUT", "/tmp/yugioh-agov-jp-production-proof-v4.json")
