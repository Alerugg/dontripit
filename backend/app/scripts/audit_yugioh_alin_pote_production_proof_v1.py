...
SURFACES = {
    "ALIN": {"idExpansion": "6025", "set_code": "ALIN", "expected": 55},
    "POTE": {"idExpansion": "5044", "set_code": "POTE", "expected": 63},
}
AGOV_EXPECTED = 98
...
def _connect():
    ...
    conn = psycopg2.connect(url, connect_timeout=30, application_name="dontripit_ygo_alin_pote_production_proof_v1")
...
def main() -> int:
    conn = _connect()
    failures = []
    report = {"status": "pass", "production_writes": 0, "surfaces": {}}
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            for label, cfg in SURFACES.items():
                ...
                wrong_method = [r for r in links if str(r.get("mapping_method") or "") != METHOD]
                ...
                canonical_exact_priceable = set()
                canonical_wrong_product = []
                if print_ids and price_as_of is not None:
                    ...
                    for row in cur.fetchall():
                        ...
                        if source_id and source_id != expected_id:
                            canonical_wrong_product.append({"print_id": print_id, "expected": expected_id, "actual": source_id})
                ...
                if wrong_method:
                    failures.append(f"{label}_wrong_method_{len(wrong_method)}")
                if wrong_confidence:
    ...
    output = os.getenv("YGO_ALIN_POTE_PRODUCTION_PROOF_OUTPUT", "/tmp/yugioh-alin-pote-production-proof-v1.json")
