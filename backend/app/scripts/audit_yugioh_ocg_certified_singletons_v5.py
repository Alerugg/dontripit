...
CERTIFIED={'INOV':'4660','TDIL':'4666','NECH':'4702','PRIO':'4709'}
...
def main():
 ...
 c=psycopg2.connect(url,connect_timeout=30,application_name='dontripit_ygo_ocg_singletons_v5'); c.set_session(readonly=True,autocommit=False)
 try:
  with c.cursor(cursor_factory=RealDictCursor) as cur:
   ...
   for code,exp in CERTIFIED.items():
    ...
    for m,g in pg.items():
     ...
     pr=pc[cid][0]
     if norm(prod.get('name'))!=norm(pr.get('card_name')): blocked['name_mismatch']+=1; continue
 ...
 Path('/tmp/yugioh-ocg-certified-singletons-v5.json').write_text(json.dumps(payload,ensure_ascii=False,indent=2,sort_keys=True,default=str)+'\n',encoding='utf-8'); print(json.dumps(payload,ensure_ascii=False,indent=2,sort_keys=True,default=str)); return 0
