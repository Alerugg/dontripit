...
def main():
 ...
 if (w['already_accepted_same_pair'],w['new_links_ready'])!=(EXPECTED_TOTAL,0):failures.append('writer_not_idempotent_223')
 url=os.getenv('DATABASE_URL_UNPOOLED') or os.getenv('DATABASE_URL')
 if not url:raise RuntimeError('DATABASE URL required')
 c=psycopg2.connect(url,connect_timeout=30,application_name='dontripit_ygo_ocg_next_singleton_heavy223_proof_v2');c.set_session(readonly=True,autocommit=False)
 try:
 ...
 if len(links)!=EXPECTED_TOTAL or len({int(r['external_product_id']) for r in links})!=EXPECTED_TOTAL or len({int(r['print_id']) for r in links})!=EXPECTED_TOTAL:failures.append('global_not_223')
 ...
 out=Path(os.getenv('YGO_OCG_NEXT_SINGLETON_HEAVY223_PROOF_OUTPUT','/tmp/ygo-ocg-next-singleton-heavy223-proof-v2.json'));out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)+'\n');print(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str));return 0 if not failures else 2
