...
GAME='yugioh'; ACCEPTED=('accepted','mapped','exact'); METHOD='cardmarket_ocg_certified_unique_physical_v8'; EXPECTED=218
SURFACES={'4680':('DOCS',69),'4725':('LTGY',75),'4809':('CSOC',74)}
...
def main()->int:
 ...
 conn=psycopg2.connect(url,connect_timeout=30,application_name='dontripit_ygo_ocg_singletons_proof_v6'); conn.set_session(readonly=True,autocommit=False)
 try:
 ...
 if len(links)!=EXPECTED or len({int(r['external_product_id']) for r in links})!=EXPECTED or len({int(r['print_id']) for r in links})!=EXPECTED: failures.append('global_identity_not_218_one_to_one')
 ...
 out=Path(os.getenv('YGO_OCG_CERTIFIED_SINGLETONS_PROOF_V6_OUTPUT','/tmp/yugioh-ocg-certified-singletons-production-v6.json')); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)+'\n',encoding='utf-8'); print(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)); return 0 if not failures else 2
