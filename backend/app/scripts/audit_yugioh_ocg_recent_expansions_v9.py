...
TARGETS={'DOCS':('4680','1677'),'LTGY':('4725','1443'),'CSOC':('4809','1156'),'SPTR':('4698','1591')}

def probe_candidate(rows,set_code,expansion_id):
 products=[r for r in rows if str(r.expansion_id or '')==expansion_id]; distinct=[]; seen=set()
 for row in products:
...
def main():
 ...
 for code,candidates in TARGETS.items():
  tried=[]; certified=None
  for exp in candidates:
   r=probe_candidate(rows,code,exp); tried.append(r)
   if r['status']=='certified': certified=exp; break
   if r['status']=='inconclusive': break
   time.sleep(1)
  reports.append({'set_code':code,'candidate_expansion_code':f'{code}-JP','candidate_ids':list(candidates),'certified_idExpansion':certified,'status':'certified' if certified else 'inconclusive','candidates':tried}); time.sleep(1.5)
 ...
 Path('/tmp/yugioh-ocg-recent-expansions-v9.json').write_text(json.dumps(payload,indent=2,sort_keys=True)+'\n',encoding='utf-8'); print(json.dumps(payload,indent=2,sort_keys=True)); return 0
