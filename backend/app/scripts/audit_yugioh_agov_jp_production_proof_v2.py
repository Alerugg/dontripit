...
def main() -> int:
    ...
    unexpected = {k: v for k, v in methods.items() if k not in EXPECTED_METHODS and int(v or 0) > 0}
    if unexpected:
        failures.append(f"unexpected_mapping_methods_{unexpected}")

    report["proof_version"] = 2
    report["expected_mapping_methods"] = EXPECTED_METHODS
    report["failures"] = failures
    report["status"] = "pass" if not failures else "fail"
    report["production_writes"] = 0

    output = os.getenv("YGO_AGOV_JP_PRODUCTION_PROOF_V2_OUTPUT", "/tmp/yugioh-agov-jp-production-proof-v2.json")
