...
GAME='yugioh'; ACCEPTED=('accepted','mapped','exact'); METHOD='cardmarket_ocg_certified_public_code_singleton_v2'; EXPECTED=168
SURFACES={'4535':('SD41',43),'4545':('SD40',38),'4557':('SD38',43),'4579':('SD36',44)}
...
def main()->int:
 ...
 conn=psycopg2.connect(url,connect_timeout=30,application_name='dontripit_ygo_ocg_structure_deck_singletons_proof_v1'); conn.set_session(readonly=True,autocommit=False)
 try:
 ...
 if len(links)!=EXPECTED or len({int(r['external_product_id']) for r in links})!=EXPECTED or len({int(r['print_id']) for r in links})!=EXPECTED: failures.append('global_identity_not_168_one_to_one')
 ...
 out=Path(os.getenv('YGO_OCG_STRUCTURE_DECK_SINGLETONS_PROOF_OUTPUT','/tmp/yugioh-ocg-structure-deck-singletons-production-v1.json')); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)+'\n',encoding='utf-8'); print(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)); return 0 if not failures else 2
