...
EXPECTED=460
SURFACES={'4870':('BE2',250),'4756':('BE02',210)}
...
def main()->int:
    ...
    conn=psycopg2.connect(url,connect_timeout=30,application_name='dontripit_ygo_ocg_beginner_editions_proof_v1')
    ...
    if len(links)!=EXPECTED or len({int(r['external_product_id']) for r in links})!=EXPECTED or len({int(r['print_id']) for r in links})!=EXPECTED:
        failures.append('global_identity_not_460_one_to_one')
    ...
    out=Path(os.getenv('YGO_OCG_BEGINNER_EDITIONS_PROOF_OUTPUT','/tmp/yugioh-ocg-beginner-editions-production-v1.json'))
