...
from app.scripts.apply_yugioh_ocg_full_bijection_cohort_v1 import TARGETS, EXPECTED_CAPTURE, EXPECTED_TOTAL
from app.scripts.apply_yugioh_ocg_full_bijection_cohort_v2 import METHOD, STABLE_IDENTITY_SHA256, run as writer_run
...
def main()->int:
    w=writer_run(False,'')
    failures=[]
    if w['status']!='pass' or w['production_writes']!=0 or w['stable_identity_sha256']!=STABLE_IDENTITY_SHA256: failures.append('writer_guard_failed')
    if (w['already_accepted_same_pair'],w['new_links_ready'])!=(EXPECTED_TOTAL,0): failures.append('writer_not_idempotent_715')
    url=os.getenv('DATABASE_URL_UNPOOLED') or os.getenv('DATABASE_URL')
    if not url: raise RuntimeError('DATABASE URL required')
    c=psycopg2.connect(url,connect_timeout=30,application_name='dontripit_ygo_ocg_cohort_proof_v2'); c.set_session(readonly=True,autocommit=False)
    try:
    ...
    for code,cfg in TARGETS.items():
      exp=str(cfg['idExpansion']); n=int(cfg['physical']); rows=[r for r in links if str(r['expansion_external_id'])==exp]
    ...
    if len(links)!=EXPECTED_TOTAL or len({int(r['external_product_id']) for r in links})!=EXPECTED_TOTAL or len({int(r['print_id']) for r in links})!=EXPECTED_TOTAL: failures.append('global_not_715_one_to_one')
    ...
    for r in links:
      ...
      rows=canon.get(int(r['print_id']),[])
      exact=[x for x in rows if str((x.get('raw_json') or {}).get('idProduct') or '')==str(r['id_product'])]
      mismatch=[x for x in rows if str((x.get('raw_json') or {}).get('idProduct') or '') not in ('',str(r['id_product']))]
      wrong+=bool(mismatch)
      if exact and any(meaningful(x) for x in exact): canon_exact+=1
      else:
        missing_canon+=1
        if len(samples)<25: samples.append({'set_code':r['set_code'],'collector_number':r['collector_number'],'idProduct':str(r['id_product']),'print_id':int(r['print_id']),'external_current_meaningful':bool(er and meaningful(er))})
    ...
    out=Path(os.getenv('YGO_OCG_COHORT_PROOF_V2_OUTPUT','/tmp/ygo-ocg-cohort-proof-v2.json')); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)+'\n'); print(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)); return 0 if not failures else 2
