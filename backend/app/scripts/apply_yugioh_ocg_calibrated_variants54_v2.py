...
METHOD='cardmarket_ocg_certified_independent_ordinal_rarity_v1'; CONFIRM='APPLY_YUGIOH_OCG_CALIBRATED_VARIANTS54_V1'
STABLE_IDENTITY_SHA256='f864e644d3b27e8adcf0a09bdb124a4735736b72134c719e5ff9931664efa72e'
TARGETS={'SLT1':('4543',26),'SR14':('5475',10),'SR13':('5111',10),'ST19':('4585',4),'SD35':('4600',4)}
ALLOWED={('secret','super'):('super','secret'),('secret','ultra'):('ultra','secret')}; EXPECTED_TOTAL=54; MIN_SUPPORT=2
...
def connect(ro):
 ...
 c=psycopg2.connect(u,connect_timeout=30,application_name='dontripit_ygo_ocg_calibrated_variants54_v2'); c.set_session(readonly=ro,autocommit=False); return c
...
def derive(cur):
 ...
 # Identity bridge/conflict guard uses every accepted Cardmarket claim, independent of calibration quality.
 cur.execute("""SELECT e.id external_product_id,e.external_id id_product,e.metacard_external_id,l.mapping_method,l.confidence,l.reviewed,p.id print_id,p.card_id FROM external_catalog_print_links l JOIN external_catalog_products e ON e.id=l.external_product_id JOIN prints p ON p.id=l.print_id WHERE e.source='cardmarket' AND e.game_id=%s AND e.product_group='single' AND l.link_status=ANY(%s)""",(gid,list(ACCEPTED)))
