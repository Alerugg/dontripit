...
def _probe_candidate(rows, set_code: str, expansion_id: str) -> dict:
    ...
    for row in distinct:
        ...
        probes.append(
            {
                "idProduct": str(row.product_id),
                "name": row.name,
                "idMetacard": str(row.metacard_id or ""),
    ...
    return {
        ...
        "status": "certified" if positives >= 2 else ("inconclusive" if any_403 else "not_certified"),
    }
...
def main() -> int:
    ...
    for set_code, candidates in TARGETS.items():
        ...
        for expansion_id in candidates:
            ...
            if report["status"] == "inconclusive":
                break
            time.sleep(1.0)
        status = "conflict" if conflict else ("certified" if certified else "inconclusive")
        reports.append(
            {
                "set_code": set_code,
                "candidate_expansion_code": f"{set_code}-JP",
                "candidate_ids": list(candidates),
                "certified_idExpansion": certified,
                "status": status,
    ...
    payload = {
        "source": "cardmarket",
        "mode": "read_only",
        "method": "global_surface_candidate_plus_first_party_cardmarket_image_s3_binary_signature",
        "production_writes": 0,
        "certified": sum(r["status"] == "certified" for r in reports),
        "conflicts": sum(r["status"] == "conflict" for r in reports),
        "inconclusive": sum(r["status"] == "inconclusive" for r in reports),
        "results": reports,
    }
    output = Path("/tmp/yugioh-ocg-recent-expansions-v2.json")
