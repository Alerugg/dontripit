...
from app.scripts.apply_yugioh_ocg_near_exact_singleton302_v1 import (
    EXPECTED_CAPTURE, EXPECTED_TOTAL, METHOD, STABLE_IDENTITY_SHA256, TARGETS, run as writer_run,
)
...
def main()->int:
    ...
    if (w['already_accepted_same_pair'],w['new_links_ready'])!=(EXPECTED_TOTAL,0): failures.append('writer_not_idempotent_302')
    url=os.getenv('DATABASE_URL_UNPOOLED') or os.getenv('DATABASE_URL')
    if not url: raise RuntimeError('DATABASE URL required')
    c=psycopg2.connect(url,connect_timeout=30,application_name='dontripit_ygo_ocg_near_exact_singleton302_proof_v1'); c.set_session(readonly=True,autocommit=False)
    try:
      with c.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        cur.execute("""SELECT l.mapping_method,l.confidence,l.reviewed,e.id external_product_id,e.external_id id_product,e.expansion_external_id,e.last_seen_at,
          ...
          WHERE e.source='cardmarket' AND e.game_id=%s AND e.product_group='single' AND l.link_status=ANY(%s) AND l.mapping_method=%s
          ORDER BY e.expansion_external_id,e.external_id::bigint""",(gid,list(ACCEPTED),METHOD)); links=[dict(r) for r in cur.fetchall()]
    ...
    for code,cfg in TARGETS.items():
      n=int(cfg['singletons']); rows=[r for r in links if str(r['expansion_external_id'])==str(cfg['exp'])]
      ...
      per[code]={'idExpansion':cfg['exp'],**d}
    if len(links)!=EXPECTED_TOTAL or len({int(r['external_product_id']) for r in links})!=EXPECTED_TOTAL or len({int(r['print_id']) for r in links})!=EXPECTED_TOTAL: failures.append('global_not_302')
    ...
    out=Path(os.getenv('YGO_OCG_NEAR_EXACT_SINGLETON302_PROOF_OUTPUT','/tmp/ygo-ocg-near-exact-singleton302-proof.json')); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)+'\n'); print(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)); return 0 if not failures else 2
