...
from app.scripts.apply_yugioh_ocg_near_exact_calibrated28_v1 import (
...
def main()->int:
    ...
    if (w['already_accepted_same_pair'],w['new_links_ready'])!=(EXPECTED_TOTAL,0): failures.append('writer_not_idempotent_28')
    url=os.getenv('DATABASE_URL_UNPOOLED') or os.getenv('DATABASE_URL')
    if not url: raise RuntimeError('DATABASE URL required')
    c=psycopg2.connect(url,connect_timeout=30,application_name='dontripit_ygo_ocg_near_exact_calibrated28_proof_v1'); c.set_session(readonly=True,autocommit=False)
    try:
    ...
    for code,(exp,n) in TARGETS.items():
      rows=[r for r in links if str(r['expansion_external_id'])==str(exp)]
    ...
    if len(links)!=EXPECTED_TOTAL or len({int(r['external_product_id']) for r in links})!=EXPECTED_TOTAL or len({int(r['print_id']) for r in links})!=EXPECTED_TOTAL: failures.append('global_not_28')
    ...
    out=Path(os.getenv('YGO_OCG_NEAR_EXACT_CALIBRATED28_PROOF_OUTPUT','/tmp/ygo-ocg-near-exact-calibrated28-proof.json')); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)+'\n'); print(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)); return 0 if not failures else 2
