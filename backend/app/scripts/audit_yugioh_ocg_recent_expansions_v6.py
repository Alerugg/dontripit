...
def main():
 ...
 for set_code,candidates in TARGETS.items():
  ...
  for exp in candidates:
   r=probe_candidate(rows,set_code,exp); tried.append(r)
   if r['status']=='certified': certified=exp; break
   if r['status']=='inconclusive': break
   time.sleep(1)
  reports.append({'set_code':set_code,'candidate_expansion_code':f'{set_code}-JP','candidate_ids':list(candidates),'certified_idExpansion':certified,'status':'certified' if certified else 'inconclusive','candidates':tried}); time.sleep(1.5)
 ...
 Path('/tmp/yugioh-ocg-recent-expansions-v6.json').write_text(json.dumps(payload,indent=2,sort_keys=True)+'\n',encoding='utf-8'); print(json.dumps(payload,indent=2,sort_keys=True)); return 0
