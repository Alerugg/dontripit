...
NATURAL_MAX_MS = 1500.0
HP_RANGE_MAX_MS = 1500.0
GLOBAL_TIMED_MAX_MS = 1800.0
...
def run(*, report_path: Path | None = None) -> dict:
    ...
    with db.SessionLocal() as session:
        ...
        for query, expected_name in (("Pikachu", "pikachu"), ("Charizard", "charizard")):
            result, elapsed = _timed(normal_pokemon_search, session, query=query, limit=12)
            ...
            _assert(elapsed <= NATURAL_MAX_MS, f"Natural search {query!r} too slow: {elapsed}ms > {NATURAL_MAX_MS}ms")
        ...
        _assert(elapsed <= HP_RANGE_MAX_MS, f"HP range too slow: {elapsed}ms > {HP_RANGE_MAX_MS}ms")
    ...
    _assert(max_ms is None or max_ms <= GLOBAL_TIMED_MAX_MS, f"A Search V2 benchmark exceeded {GLOBAL_TIMED_MAX_MS}ms: max={max_ms}ms")
    report = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "game": "pokemon",
        "status": "pass",
        "latency_gates_ms": {
            "natural_search": NATURAL_MAX_MS,
            "hp_range": HP_RANGE_MAX_MS,
            "global_timed_check": GLOBAL_TIMED_MAX_MS,
        },
        "checks": checks,
        "performance": {
