...
GAME='yugioh'; ACCEPTED=('accepted','mapped','exact'); METHOD='cardmarket_ocg_certified_unique_physical_v6'; EXPECTED=256
SURFACES={'4627':('FLOD',64),'4634':('EXFO',64),'4640':('CIBR',64),'4655':('RATE',64)}
...
def main()->int:
 ...
 conn=psycopg2.connect(url,connect_timeout=30,application_name='dontripit_ygo_ocg_singletons_proof_v4'); conn.set_session(readonly=True,autocommit=False)
 try:
 ...
 if len(links)!=EXPECTED or len({int(r['external_product_id']) for r in links})!=EXPECTED or len({int(r['print_id']) for r in links})!=EXPECTED: failures.append('global_identity_not_256_one_to_one')
 ...
 out=Path(os.getenv('YGO_OCG_CERTIFIED_SINGLETONS_PROOF_V4_OUTPUT','/tmp/yugioh-ocg-certified-singletons-production-v4.json')); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)+'\n',encoding='utf-8'); print(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)); return 0 if not failures else 2
