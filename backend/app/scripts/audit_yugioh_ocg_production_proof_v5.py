...
EXPECTED={
    "DUAD":{"exp":"6129","set":"DUAD","count":38,"methods":{
        "cardmarket_ocg_certified_unique_physical_v2":38,
    }},
    "ALIN":{"exp":"6025","set":"ALIN","count":132,"methods":{
...
def main()->int:
    ...
    conn=psycopg2.connect(url,connect_timeout=30,application_name="dontripit_ygo_ocg_production_proof_v5")
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            for label,cfg in EXPECTED.items():
                ...
                unpriced=[{'idProduct':str(r['id_product']),'print_id':int(r['print_id']),'collector_number':r['collector_number'],'card_name':r['card_name'],'variant':r['variant'],'rarity':r['rarity']} for r in links if int(r['external_product_id']) not in ext_priceable]
    ...
    out=Path(os.getenv('YGO_OCG_PRODUCTION_PROOF_V5_OUTPUT','/tmp/yugioh-ocg-production-proof-v5.json'))
