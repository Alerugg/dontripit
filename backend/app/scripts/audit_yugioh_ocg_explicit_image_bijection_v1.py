...
def connect():
    ...
    conn = psycopg2.connect(url, connect_timeout=30, application_name="dontripit_ygo_ocg_explicit_image_bijection_v1")
    conn.set_session(readonly=True, autocommit=False)
    return conn


def main() -> int:
    parser = argparse.ArgumentParser(description="READ ONLY certify one explicit OCG 2x2 physical variant group by first-party images")
    ...
    parser.add_argument("--id-product", action="append", required=True)
    parser.add_argument("--print-id", action="append", type=int, required=True)
    parser.add_argument("--report", type=Path, required=True)
    args = parser.parse_args()
    if len(args.id_product) != 2 or len(set(args.id_product)) != 2:
        raise RuntimeError("explicit certifier requires exactly two distinct idProduct values")
    if len(args.print_id) != 2 or len(set(args.print_id)) != 2:
        raise RuntimeError("explicit certifier requires exactly two distinct print IDs")
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            cur.execute(
                """SELECT e.id external_product_id,e.external_id id_product,e.name,e.category_id,e.metacard_external_id,e.expansion_external_id
                   FROM external_catalog_products e
                   WHERE e.source='cardmarket' AND e.game_id=%s AND e.product_group='single'
                     AND e.external_id=ANY(%s) AND e.last_seen_at=%s
                   ORDER BY e.external_id::bigint""",
                (game_id, list(args.id_product), capture),
            )
            ...
            cur.execute(
                ...
                (game_id, list(ACCEPTED), product_row_ids or [-1], list(args.print_id)),
            )
    ...
    if len(products) != 2 or {str(r["id_product"]) for r in products} != set(args.id_product):
        raise RuntimeError({"explicit_products_missing_or_drifted": products})
    if len(prints) != 2 or {int(r["print_id"]) for r in prints} != set(args.print_id):
        raise RuntimeError({"explicit_prints_missing_or_drifted": prints})
    if claims:
        raise RuntimeError({"explicit_surface_already_claimed": claims})
    if {str(r.get("expansion_external_id") or "") for r in products} != {args.id_expansion}:
        raise RuntimeError("product expansion drift")
    metas = {str(r.get("metacard_external_id") or "") for r in products}
    if len(metas) != 1 or "" in metas:
        raise RuntimeError({"products_do_not_share_one_nonempty_metacard": sorted(metas)})
    if len({int(r["card_id"]) for r in prints}) != 1:
        raise RuntimeError("canonical prints do not share one card")
    if {str(r.get("set_code") or "").upper() for r in prints} != {args.set_code.upper()}:
        raise RuntimeError("canonical set drift")
    if {str(r.get("language") or "").casefold() for r in prints} != {"ja"}:
        raise RuntimeError("canonical language drift")
    card_names = {str(r.get("card_name") or "") for r in prints}
    if len(card_names) != 1:
        raise RuntimeError("canonical names drift")
    canonical_name = next(iter(card_names))
    if any(norm(r.get("name")) != norm(canonical_name) for r in products):
        raise RuntimeError({"strict_normalized_name_mismatch": {str(r['id_product']): r.get('name') for r in products}, "canonical": canonical_name})
    ...
    complete = len(product_features) == 2 and len(print_features) == 2
    matrix = []
    if complete:
    ...
    if consensus and assignments["pixel_mae"]["best"]:
        for pair in assignments["pixel_mae"]["best"]["pairs"]:
            pid = str(pair["idProduct"]); print_id = int(pair["print_id"])
            distance = lookup[(pid, print_id)]; pm = product_meta[pid]; cm = print_meta[print_id]
    ...
    report = {
        ...
        "certified_region": {"idExpansion": args.id_expansion, "expansion_code": args.expansion_code, "canonical_set": args.set_code.upper()},
        "card_name": canonical_name,
        "product_names": sorted({str(r.get("name") or "") for r in products}),
