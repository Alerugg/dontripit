...
from app.scripts.apply_yugioh_ocg_full_bijection_cohort_v1 import TARGETS, METHOD, EXPECTED_CAPTURE, EXPECTED_TOTAL
...
def main()->int:
    ...
    conn=psycopg2.connect(url,connect_timeout=30,application_name='dontripit_ygo_ocg_full_bijection_cohort_proof_v1')
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            cur.execute("""SELECT l.mapping_method,l.confidence,l.reviewed,e.id external_product_id,e.external_id id_product,e.expansion_external_id,e.last_seen_at,
                ...
                ORDER BY e.expansion_external_id,e.external_id::bigint""",(gid,exps,list(ACCEPTED))); links=[dict(r) for r in cur.fetchall()]
    ...
    if len(links)!=EXPECTED_TOTAL or len({int(r['external_product_id']) for r in links})!=EXPECTED_TOTAL or len({int(r['print_id']) for r in links})!=EXPECTED_TOTAL:
        failures.append('global_identity_not_715_one_to_one')
    ...
    out=Path(os.getenv('YGO_OCG_FULL_BIJECTION_COHORT_PROOF_OUTPUT','/tmp/ygo-ocg-full-bijection-cohort-production-v1.json')); out.parent.mkdir(parents=True,exist_ok=True); out.write_text(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)+'\n',encoding='utf-8'); print(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True,default=str)); return 0 if not failures else 2
