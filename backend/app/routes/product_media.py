...
def _exact_cardmarket_source(variant_id: int) -> dict | None:
    with db.SessionLocal() as session:
        rows = session.execute(_EXACT_CARDMARKET_VARIANT_SQL, {"variant_id": variant_id}).mappings().all()
    if len(rows) != 1:
        return None
    row = dict(rows[0])
...
def _fetch_exact_image(url: str, timeout: int = 12) -> tuple[bytes, str]:
    ...
    with urllib.request.urlopen(request, timeout=timeout) as upstream:
        if int(getattr(upstream, "status", 200) or 200) != 200:
            raise urllib.error.HTTPError(url, int(upstream.status), "unexpected status", upstream.headers, None)
...
def exact_cardmarket_product_image(variant_id: int):
    ...
    try:
        body, content_type = _fetch_exact_image(url)
    ...
    response.headers["X-Image-Source"] = "cardmarket-exact"
    return response
