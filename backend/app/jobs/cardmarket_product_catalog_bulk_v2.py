...
def run() -> dict:
    ...
    try:
        with conn.cursor() as cur:
            ...
            products_inserted = int(cur.rowcount)
            cur.execute(
                """
                INSERT INTO product_variants (id,product_id,language,region,packaging,sku)
                SELECT s.variant_id,s.product_id,s.language,s.region,s.packaging,left('cardmarket:'||s.external_id,100)
                FROM cm_product_stage s
                WHERE NOT EXISTS (SELECT 1 FROM product_variants pv WHERE pv.id=s.variant_id)
                """
            )
            variants_inserted = int(cur.rowcount)
            cur.execute(
                """
            ...
            identifiers_inserted = int(cur.rowcount)
            ...
            links_upserted = int(cur.rowcount)
            ...
            price_snapshots_upserted = int(cur.rowcount)
            ...
            cur.execute(
                ...
                SELECT count(*) FROM cm_product_stage s
                JOIN external_catalog_product_variant_links l
                  ON l.external_product_id=s.external_product_id AND l.product_variant_id=s.variant_id
            ...
            result = {
                ...
                "accepted_links_upserted": links_upserted,
                "price_snapshots_upserted": price_snapshots_upserted,
                "missing_price_products": missing_price_products,
                "ambiguous_price_variants": ambiguous_prices,
