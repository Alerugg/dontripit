    ...
    *,
    ...
    if key == "release":
        rows = session.execute(
            text(
                """
                SELECT rel AS value, rel AS label, COUNT(*) AS count
                FROM print_search_profiles psp
                JOIN games g ON g.id = psp.game_id
                CROSS JOIN LATERAL jsonb_array_elements_text(COALESCE(psp.release_names_json, '[]'::jsonb)) rel
                WHERE g.slug = :game
                  AND (:q = '%%' OR lower(rel) LIKE :q)
                GROUP BY rel
                ORDER BY count DESC, rel ASC
                LIMIT :limit
                """
            ),
            params,
    ...
    raise ValueError(f"Facet values are not available for: {key}")
