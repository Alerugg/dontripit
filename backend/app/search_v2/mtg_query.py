...
def normal_mtg_search(session, *, query: str, limit: int = 24) -> list[dict]:
    """Fast logical-Card MTG search without hydrating the print universe.

    Human name searches start from the much smaller Card projection. A first
    pass uses exact/prefix/contains and surviving query tokens as cheap anchors;
    only when that returns nothing do we pay for a broad trigram-similarity
    fallback. Print-profile signals are reserved for code-shaped input because
    exact identifier search already handles known set/collector codes upstream.
    """
    ...
    code_shaped = any(ch.isdigit() for ch in q_norm) or ("-" in raw_query and len(tokens) <= 2)
    params: dict[str, object] = {
        ...
        "contains": f"%{q_norm}%",
        "limit": bounded_limit,
        "candidate_limit": max(60, bounded_limit * 5),
    }
    ...
    for index, token in enumerate(tokens):
        ...
        if len(token) >= 3:
            token_card_checks.append(f"csp.search_text LIKE :token_like_{index}")
    ...
    token_anchor_predicate = " OR ".join(token_card_checks) or "FALSE"
    print_token_predicate = " OR ".join(token_print_checks) or "FALSE"

    def build_sql(*, broad_similarity: bool, include_print_signal: bool):
        similarity_predicate = "OR similarity(csp.normalized_name,:q_norm) >= 0.20" if broad_similarity else ""
        print_cte = ""
        print_union = ""
        if include_print_signal:
            print_cte = f""",
            ...
            print_union = "UNION ALL SELECT card_id,score FROM print_candidates"
        ...
        return text(
            ...
            SELECT
              ...
              chosen.primary_image_url,chosen.variant_count,top_candidates.score
            FROM top_candidates
            JOIN cards c ON c.id=top_candidates.card_id
            ...
            JOIN LATERAL (
              ...
              WHERE psp.card_id=top_candidates.card_id
              ORDER BY
            ...
            ORDER BY top_candidates.score DESC,c.name ASC,c.id ASC
            """
        )

    rows = session.execute(
        build_sql(broad_similarity=False, include_print_signal=code_shaped),
        params,
    ).mappings().all()
    if not rows:
        rows = session.execute(
            build_sql(broad_similarity=True, include_print_signal=code_shaped),
            params,
        ).mappings().all()
