...
def test_material_state_preserves_known_dates_when_source_omits_them():
    record = _record_for_source(_source(), _item(published_date=None, release_date=None), "us")
    current = {
...
def test_workflow_gates_before_writes_and_certifies_second_pass():
    ...
    workflow = (repo_root / ".github/workflows/sync-regional-tcg-content.yml").read_text(encoding="utf-8")

    dry_run = workflow.index("--dry-run")
    migration = workflow.index("alembic upgrade head")
    apply = workflow.index("--apply")
    cleanup = workflow.index("--cleanup-deprecated")
    push_guard = workflow.rfind(
        "if: github.event_name == 'push' && github.ref_name == 'catalog-v2'",
        0,
        cleanup,
    )

    assert dry_run < migration < apply < cleanup
