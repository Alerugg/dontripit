...
def build_plan(conn, min_samples: int) -> dict:
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        official_rows = list(cur.fetchall())
        ...
        all_existing_links = list(cur.fetchall())
    ...
    for row in products:
        ...
        if not expansion_id:
            continue
        all_external_by_expansion[expansion_id].append(dict(row))
    ...
    candidates = [row for row in candidates if row["print_id"] not in duplicate_target_conflicts]

    links_by_external: dict[int, list[dict]] = defaultdict(list)
    for row in all_existing_links:
        links_by_external[int(row["external_product_id"])].append(dict(row))

    blocked_existing_links = []
    safe_candidates = []
    for row in candidates:
        existing = links_by_external.get(row["external_product_id"], [])
        if existing:
            blocked_existing_links.append(
                {
                    "candidate": row,
                    "existing_links": existing,
                    "reason": "external_product_already_has_nonaccepted_or_conflicting_link",
                }
            )
            continue
    ...
    return {
        ...
        "summary": {
            ...
            "accepted_exact_external_products_before": len(accepted_external_ids),
            "accepted_exact_target_prints_before": len(accepted_print_ids),
            ...
            "candidate_links_before_duplicate_guard": len(target_to_external) + sum(
                len(v) - 1 for v in duplicate_target_conflicts.values()
            ),
            "duplicate_target_conflicts": len(duplicate_target_conflicts),
            "blocked_existing_links": len(blocked_existing_links),
        ...
        "blocked_existing_links": blocked_existing_links,
    }
...
def apply_plan(conn, plan: dict) -> dict:
    ...
    for item in plan["safe_candidates"]:
        evidence = {
            ...
            "identity_basis": [
                ...
                "official_konami_release_membership",
            ...
            "cardmarket_idProduct": item["cardmarket_id_product"],
            "cardmarket_expansion_id": item["cardmarket_expansion_id"],
            "konami_pid": item["konami_pid"],
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            ...
            FROM external_catalog_print_links l
            JOIN external_catalog_products ecp ON ecp.id=l.external_product_id
            WHERE ecp.source='cardmarket' AND ecp.game_id=%s
              AND ecp.product_group='single'
              AND l.mapping_method='ygo_konami_release_exact'
              AND l.link_status='accepted' AND l.confidence='exact'
            """,
            (plan["game_id"],),
        )
        mapped_rows, mapped_products, mapped_prints = map(int, cur.fetchone())
        cur.execute(
            """
            SELECT count(DISTINCT l.external_product_id), count(DISTINCT l.print_id)
            FROM external_catalog_print_links l
            JOIN external_catalog_products ecp ON ecp.id=l.external_product_id
            WHERE ecp.source='cardmarket' AND ecp.game_id=%s
              AND ecp.product_group='single'
        ...
        accepted_products_after, accepted_prints_after = map(int, cur.fetchone())

    expected_min = len(plan["safe_candidates"])
    if mapped_products < expected_min or mapped_prints < expected_min:
        raise AssertionError(
            {
                "reason": "release-aware mapping postcondition failed",
    ...
    return {
        "release_aware_mapping_rows": mapped_rows,
        "release_aware_products": mapped_products,
        "release_aware_prints": mapped_prints,
        "accepted_exact_products_after": accepted_products_after,
        "accepted_exact_prints_after": accepted_prints_after,
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="Apply release-aware exact YGO Cardmarket links.")
    ...
    try:
        plan = build_plan(conn, args.min_samples)
        result = {
            "mode": "apply" if args.apply else "dry_run",
            "game": "yugioh",
            "mapping_method": "ygo_konami_release_exact",
            "write_ready": True,
            **plan["summary"],
            "safe_candidates": plan["safe_candidates"],
            "blocked_existing_links_detail": plan["blocked_existing_links"],
            "duplicate_target_conflicts_detail": plan["duplicate_target_conflicts"],
        }
        if args.apply:
            result.update(apply_plan(conn, plan))
            conn.commit()
        else:
            conn.rollback()
